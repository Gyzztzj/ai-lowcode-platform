  ...
  if (!currentConversation) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-gray-500">
        ...
        <p className="text-sm mb-4">{currentApp.description || '开始你的第一个对话'}</p>
        <p className="text-xs text-gray-400">点击左侧"新对话"按钮创建一个新的对话</p>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6 scrollbar-thin">
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-500">
            ...
            <h3 className="text-xl font-medium mb-2">{currentApp.name}</h3>
            <p className="text-sm mb-4">{currentApp.description || '有什么可以帮助你的？'}</p>
