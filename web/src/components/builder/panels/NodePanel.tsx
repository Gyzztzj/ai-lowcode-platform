...
const NodePanel = () => {
  ...
  const showKnowledgeBaseNode =
    currentApp && currentApp.embeddingModel && currentApp.embeddingModel !== 'none';
  ...
  const handleDragStart = (e: React.DragEvent, type: string, label: string, color: string) => {
    e.dataTransfer.setData('application/reactflow', JSON.stringify({ type, label, color }));
  };
