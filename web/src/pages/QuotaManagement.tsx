...
import { useUserQuotas, useAppQuotas, useQuotaMutations } from '@/hooks/useApi';
...
const QuotaManagement = () => {
  ...
  const { data: users = [], isPending: usersLoading } = useUserQuotas();
  const { data: apps = [], isPending: appsLoading } = useAppQuotas();
  const {
    updateUserQuota,
    updateAppQuota,
    isUpdateUserQuotaPending,
    isUpdateAppQuotaPending,
  } = useQuotaMutations();
  ...
  const isLoading = usersLoading || appsLoading || isUpdateUserQuotaPending || isUpdateAppQuotaPending;
    ...
    try {
      if ('email' in selectedItem) {
        await updateUserQuota({
          id: selectedItem.id,
          data: { dailyQuota, monthlyQuota },
        });
      } else {
        await updateAppQuota({
          id: selectedItem.id,
          data: { dailyQuota, monthlyQuota },
        });
      }
  ...
  const displayedItems = showUsers ? users : apps;
  const displayedLoading = showUsers ? usersLoading : appsLoading;
    ...
    <div className="p-6 space-y-6">
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex justify-between mt-1">
                          ...
                          >
                            {getUsageStatus(calculateUsagePercent(user.dailyUsed, user.dailyQuota)).label}
                          </span>
                        </div>
                      </div>
                      <div>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      {!app.dailyQuota && !app.monthlyQuota && (
                        <p className="text-xs text-gray-500">当前使用用户级配额设置</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
              ))}
          {displayedItems.length === 0 && !displayedLoading && (
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{showUsers && selectedItem && 'email' in selectedItem ? '编辑用户配额' : '编辑应用配额'}</DialogTitle>
            <DialogDescription>
              设置{showUsers ? '用户' : '应用'}的每日和每月API调用配额
            </DialogDescription>
          </DialogHeader>
